...
import (
	...
	"kroseida.org/slixx/internal/supervisior/datasource/provider"
...
func CreateSatellite(ctx context.Context, args CreateSatelliteDto) (*Satellite, error) {
	...
	reactive.InvalidateAfter(ctx, 5*time.Second)
	satellite, err := datasource.SatelliteProvider.CreateSatellite(args.Name, args.Description, args.Address, args.Token)
	if err != nil {
		application.Logger.Debug(err)
		return nil, err
	}
...
func UpdateSatellite(ctx context.Context, args UpdateSatelliteDto) (*Satellite, error) {
	...
	reactive.InvalidateAfter(ctx, 5*time.Second)
	satellite, err := datasource.SatelliteProvider.UpdateSatellite(args.Id, args.Name, args.Description, args.Address, args.Token)
	if err != nil {
		application.Logger.Debug(err)
		return nil, err
	}
...
func DeleteSatellite(ctx context.Context, args DeleteSatelliteDto) (*Satellite, error) {
	...
	reactive.InvalidateAfter(ctx, 5*time.Second)
	satellite, err := datasource.SatelliteProvider.DeleteSatellite(args.Id)
	if err != nil {
		application.Logger.Debug(err)
		return nil, err
	}
